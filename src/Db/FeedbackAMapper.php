...
class FeedbackAMapper implements AMapper
{
    private $factory;
    ...
    private function createLocalRefField(AItem $owner, RefField $refMeta, $id)
    {
    ...
    public function getByRef(ARef $ref, $asUnitMember = false)
    {
        ...
        if(count($result) === 0)
        {
            throw new FeedbackException('При получении элемента не найдены данные в таблице '.$type_name.': по ссылке '.$ref.'!');
        }
        ...
        foreach($ownsMetaCollection as $ownMeta)
        {
            ...
            else
            {
                ...
                {
                    throw new FeedbackException('В типе '.$type_name.' обнаружено поле-собственность типа отличного от В или С: '.$field_name.'('.$fieldMode.')!');
                }
            }
        ...
        {
            $newField = $this->createLocalRefField($item, $refMeta, 0);//Пока все ссылки на блоки-формы из групп
